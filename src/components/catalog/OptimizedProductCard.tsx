...
function ProductCardComponent({ product, index = 0, priority = false }: ProductCardProps) {
  ...
  return (
    ...
    >
      ...
      <div
        className="absolute -inset-1 rounded-2xl bg-gradient-to-br from-neon-purple-500 via-neon-purple-500 to-neon-purple-600 opacity-0 group-hover:opacity-60 blur-xl transition-opacity duration-500"
        aria-hidden="true"
      />

      {/* Card container */}
      <div className="relative h-full rounded-2xl overflow-hidden bg-void-200 border border-neon-purple-500/20 group-hover:border-neon-purple-400/40 transition-colors duration-300">
        {/* Badges */}
        ...
        {discount > 0 && (
          <div className="absolute top-4 right-4 z-20">
            <div className="w-14 h-14 rounded-full bg-gradient-to-br from-red-500 to-orange-600 flex items-center justify-center shadow-[0_0_20px_rgba(239,68,68,0.6)]">
              <span className="font-inter font-black text-white text-sm">
                -{discount}%
              </span>
            </div>
          </div>
          ...
          {product.image ? (
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="relative w-48 h-48 group-hover:scale-105 transition-transform duration-500">
                <div
                  className="absolute bottom-0 left-1/2 -translate-x-1/2 w-32 h-8 bg-neon-purple-500/30 blur-xl rounded-full"
                  aria-hidden="true"
                />
                <div className="absolute inset-0 flex items-center justify-center">
                  <Monitor
        ...
        <div className="relative p-5">
          ...
          <div className="flex items-end justify-between mb-4">
            <div>
              ...
              <div
                className={cn(
                  'text-2xl font-inter font-black',
                  product.salePrice
                    ? 'text-transparent bg-gradient-to-r from-neon-purple-400 to-neon-purple-400 bg-clip-text'
                    : 'text-white'
                )}
              >
                {formattedPrice}
          ...
          >
            <button
              type="button"
              className="relative w-full py-3.5 px-6 rounded-xl overflow-hidden font-inter font-bold text-sm uppercase tracking-wider transition-transform duration-200 hover:scale-[1.02] active:scale-[0.98]"
            >
              ...
              {/* Hover overlay */}
              <div
                className="absolute inset-0 bg-gradient-to-r from-neon-purple-500 to-neon-purple-500 opacity-0 group-hover:opacity-100 transition-opacity duration-300"
                aria-hidden="true"
              />
        ...
        {/* Bottom glow line */}
        <div
          className="absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r from-neon-purple-500 via-neon-purple-500 to-neon-purple-500 opacity-0 group-hover:opacity-100 transition-opacity duration-300"
          aria-hidden="true"
        />
      </div>
    </motion.article>
