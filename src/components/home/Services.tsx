...
export function Services() {
  return (
    <section id="services" className="py-24 relative">
      ...
      <div className="absolute bottom-1/4 right-0 w-96 h-96 bg-magenta-600/20 rounded-full blur-[128px] pointer-events-none" />
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {services.map((service, index) => (
            ...
            >
              {/* Hover glow */}
              <div className="absolute inset-0 bg-gradient-to-br from-purple-600/5 to-magenta-600/5 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity" />
                ...
                <ul className="space-y-2">
                  {service.features.map((feature, i) => (
                    <li key={i} className="flex items-start gap-2 text-gray-400 text-sm">
                      <span className="w-1 h-1 rounded-full bg-gradient-to-r from-purple-500 to-magenta-500 mt-2 flex-shrink-0" />
                      <span>{feature}</span>
                    </li>
                  ))}
                </ul>
